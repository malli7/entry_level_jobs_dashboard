...
export  async function POST(req: Request) {
  ...
  try {
    ...
    if (jobDoc.exists()) {
      ...
      return NextResponse.json({ match_score: existingData.match_score, feedback: existingData.feedback });
    }
    ...
    const response = await fetch('https://job-scraper.up.railway.app/evaluate-resume', {
      method: 'POST',
      headers: {
    ...
    const data = await JSON.parse(x.slice(7,-3))

    if (data.score === undefined || data.review === undefined) {
      return NextResponse.json({ message: 'Invalid response from external API' }, { status: 500 });
    }
    ...
    return NextResponse.json({ match_score: data.score, feedback: data.review });
  } catch (error) {
